    ...
    function renderResult({ item }) {
        ...
        return (
            <TouchableOpacity style={[style.resultFound]} activeOpacity={0.4} onPress={() => goToLocation(item.cfi)}>
                <Text style={[style.resultFoundTitle, fontStyle]}>{splittedText[0]}<Text style={[style.resultFoundTitleBold, fontStyle]}>{searchedWord}</Text>{splittedText[1]}</Text>
            </TouchableOpacity>
        )
    }
                ...
                <View style={style.modalContainer}>
                    <View style={[style.resultsContainer, { backgroundColor: isDarkMode ? '#666' : '#FFF' }]}>
                        <View style={{ flexDirection: "row", alignItems: "center", marginBottom: 20 }}>
                            <TextInput value={search} placeholder='Buscar palavra' onChangeText={setSearch} style={{ padding: 10, borderRadius: 5, backgroundColor: '#fafafa', flex: 1 }} placeholderTextColor='#000' />
                            <Button title="Buscar" onPress={goSearch} style={{ width: '100%', marginRight: 20 }} />
