...
function Reader({ navigation, route }) {
    ...
    function goSearch() {
        ...
        function doSearch(q) {
            return Promise.all(
                window.book.spine.spineItems.map(item => item.load(window.book.load.bind(book)).then(item.find.bind(item, q)).finally(item.unload.bind(item)))
            ).then(results => Promise.resolve([].concat.apply([], results)));
        };
        var resultados = doSearch(${search});
        window.ReactNativeWebView.postMessage(
            JSON.stringify({ type: 'search', results: resultados })
        );
        true`);
    }
