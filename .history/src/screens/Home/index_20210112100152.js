    ...
    function renderResult({ item }) {
        ...
        return (
            <TouchableOpacity style={style.resultFound} activeOpacity={0.4} onPress={() => goToLocation(item.cfi)}>
                <Text style={style.resultFoundTitle}>{splittedText[0]}</Text>
                <Text style={style.resultFoundTitleBold}>{search}</Text>
                <Text style={style.resultFoundTitle}>{splittedText[1]}</Text>
            </TouchableOpacity>
        )
    }
