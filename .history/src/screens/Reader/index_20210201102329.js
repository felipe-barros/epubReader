...
function Reader({ navigation, route }) {
    ...
    const [isSearchingPage, setIsSerchingPage] = useState(false);
            ...
            <View style={style.footer}>
                ...
                </TouchableOpacity>
                {
                    !totalPages ? (
                        ...
                        <>
                            <TouchableOpacity activeOpacity={0.8} onPress={() => setIsSearchingPage(!isSearchingPage)} style={{ position: 'absolute', left: 100, right: 100 }} onPress={() => alert("Slider")}>
                                <Text style={[style.footerText, fontStyle]}>{progress} de {totalPages}</Text>
                            </TouchableOpacity>
                            {isSearchingPage && <View style={{ hid, width: '100%', position: 'absolute', top: -40, left: 0, right: 0, alignItems: 'center' }}>
                                <Text style={[style.footerText, fontStyle]}>Página {searchPage}</Text>
                                <Slider
                                    minimumValue={1}
