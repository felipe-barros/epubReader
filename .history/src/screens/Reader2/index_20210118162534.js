    ...
    function renderResult({ item }) {
        ...
        return (
            <TouchableOpacity style={[style.resultFound]} activeOpacity={0.4} onPress={() => goToLocation(item.cfi)}>
                <Text style={[style.resultFoundTitle, fontStyle]}>{item.excerpt.slice(0, splittedTextAux[0].length)}<Text style={[style.resultFoundTitleBold, fontStyle]}>{item.excerpt.slice(splittedTextAux[0].length, searchedWord.length)}</Text>{item.excerpt.slice(splittedTextAux[0].length + searchedWord.length, splittedTextAux[2].length)}</Text>
            </TouchableOpacity>
        )
    }
