...
function Reader({ route }) {
    const { pathIos, pathAndroid } = route.params;
    const webview = useRef();
    // JavaScript que será rodado antes da página ser renderizada
    let injectedJS = `window.BOOK_PATH = "${Platform.OS == 'ios' ? pathIos : { uri: 'file:///android_asset/book2.epub' }}"; window.THEME = ${JSON.stringify(themeToStyles(lightMode))};`;
