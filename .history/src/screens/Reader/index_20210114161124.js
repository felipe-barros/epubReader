...
function Reader({ route }) {
    ...
    useEffect(() => {
        ...
        newServer.start().then((url) => {
            // console.log(url);
            setBookUrl(url + path.replace(ExternalStorageDirectoryPath, ''))
        });
        return () => {
    ...
    console.log("New Path:", bookUrl)
    let injectedJS = `window.BOOK_PATH = "${bookUrl}"; window.THEME = ${JSON.stringify(themeToStyles(lightMode))};`;
    // console.log(injectedJS)
    // Path para o arquivo html de leitura do livro
    const pathToHtml = Platform.OS == 'ios' ? iosFilePath : { uri: 'bundle-assets://index.html' }
