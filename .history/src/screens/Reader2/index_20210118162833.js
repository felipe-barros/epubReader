...
function Home({ navigation }) {
    ...
    function renderResult({ item }) {
        const splittedTextAux = item.excerpt.toUpperCase().split(searchedWord.toUpperCase());
        console.log(item.excerpt.slice(0, splittedTextAux[0].length))
        console.log(splittedTextAux[0])
        console.log(splittedTextAux[1])

        return (
            <TouchableOpacity style={[style.resultFound]} activeOpacity={0.4} onPress={() => goToLocation(item.cfi)}>
                <Text style={[style.resultFoundTitle, fontStyle]}>{item.excerpt.slice(0, splittedTextAux[0].length)}<Text style={[style.resultFoundTitleBold, fontStyle]}>{item.excerpt.slice(splittedTextAux[0].length, searchedWord.length)}</Text>{item.excerpt.slice(splittedTextAux[0].length + searchedWord.length, splittedTextAux[1].length)}</Text>
            </TouchableOpacity>
        )
    }
        ...
        <SafeAreaView style={[style.container, themeStyle]}>
            <View style={style.content}>
                ...
                <View style={{ paddingHorizontal: 20, flex: 1 }}>
                    <WebView
                        ref={webview}
                        source={Platform.OS == 'ios' ? htmlPathIos : { uri: 'file:///android_asset/index2.html' }}
                    ...
                    <View style={[style.resultsContainer, { backgroundColor: isDarkMode ? '#666' : '#FFF' }]}>
                        ...
                        <View style={{ marginTop: 40 }}>
                            ...
                            <View style={{ flexDirection: 'row', marginTop: 20, justifyContent: 'center', alignItems: 'center' }}>
                                <TouchableOpacity style={{ marginRight: 40 }} onPress={increaseFontSize} >
