...
class default_DefaultViewManager {
  ...
  paginatedLocation() {
    ...
    let sections = visible.map(view => {
      ...
      let pages = [];
      let endPage = Math.floor(end / (vertical ? this.layout.pageHeight : this.layout.pageWidth)); // start page should not be negative
    ...
    if (!this.settings.fullsize) {
      ...
      if (x) {
        ...
        if (Math.abs(absX - remainder) >= 1) {
          ...
          const compensationDir = Math.round(remainder / absX) === 1 ? 1 : -1; // Compensate it by 1 pixel.
      ...
      if (y) {
        ...
        if (Math.abs(absY - remainder) >= 1) {
          // Determine if the current scroll position is closer to the next page or current page.
          const compensationDir = Math.round(remainder / absY) === 1 ? 1 : -1; // Compensate it by 1 pixel.
  ...
  updateLayout() {
    ...
    if (!this.isPaginated) {
      ...
      this.layout.calculate(this._stageSize.width, this._stageSize.height, this.settings.gap, this.settings.axis); // Set the look ahead offset for what is visible
...
class rendition_Rendition {
  ...
  */
  ...
  next(n = 1) {
    return this.q.enqueue(this.manager.next.bind(this.manager), n).then(this.reportLocation.bind(this));
  }
  /**
  ...
  prev(n = 1) {
    return this.q.enqueue(this.manager.prev.bind(this.manager), n).then(this.reportLocation.bind(this));
