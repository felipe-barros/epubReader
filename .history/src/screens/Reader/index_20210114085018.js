...
function Reader({ route }) {
    const { title, pathIos, pathAndroid } = route.params;
    const webview = useRef();
    // JavaScript que será rodado antes da página ser renderizada
    let injectedJS = `window.BOOK_PATH = "${Platform.OS == 'ios' ? pathIos : pathAndroid}"; window.THEME = ${JSON.stringify(themeToStyles(lightMode))};`;
